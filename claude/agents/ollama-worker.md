...
- `model`には、実際の名前の代わりに別名`fast`と`deep`を渡せます。サーバーが`DEFAULT_MODEL`と`DEEP_MODEL`に読み替えるため、PCごとのモデルの名前を覚えずに済みます。入っているモデルは`ollama_list_models`で確かめます
- `fast`（既定は`nucbox-fast:latest`、qwen2.5-coder 7B）: 速いモデルです。短い要約、言い換え、単純な雛形に向きます
- `deep`（既定は`qwen2.5-coder:14b`）: 遅いものの、より正確です。コードの確認、エラーの解析、長めの下書きに向きます
- 渡せる量の上限は、既定では約24000トークンです（コンテキストを32kトークンと見込んだ値）。`OLLAMA_NUM_CTX`を設定したサーバーでは、その値から決まります。実際のコンテキスト長は`ollama_health`の`context`と`loaded models`に出ます
