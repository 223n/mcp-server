...
async function captureAsync(run) {
  ...
  try {
    return { result: await run().catch((error) => error), lines };
  } finally {
    console.error = original;
  }
}
...
test("パスが多いときは件数を添えて切る", () => {
  ...
  assert.equal(fields.files.length, 20);
...
test("長い値は切り詰める", () => {
  ...
  assert.ok(fields.output_name.length <= 201, fields.output_name.length);
});
...
test("識別子は毎行に入る", () => {
  const { lines } = capture(() => withIdentity("you@example.com", () => audit({ kind: "resource" })));
