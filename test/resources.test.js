...
const { toFileUri } = await import("../src/tools/resources.js");
...
const readText = async (client, uri) =>
  (await client.readResource({ uri })).contents.map((c) => c.text).join("\n");
...
for (const [name, target, expected] of [
  ...
  test(`resources/read でも拒む: ${name}`, async () => {
    ...
    await assert.rejects(client.readResource({ uri: toFileUri(target()) }), (error) => {
