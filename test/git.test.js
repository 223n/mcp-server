...
test("知らない op を拒む", async () => {
  await assert.rejects(gitRead({ repo: "223n/seed", op: "archive" }), /Unknown op/);

  await assert.rejects(gitWrite({ repo: "223n/seed", op: "reset" }), /Unknown op/);
});
...
test("CLONE_ROOT が無ければ git のツールごと無効にする", async () => {
  ...
  config.cloneRoot = { hostLabel: "X", localPath: path.join(cloneRoot, "missing") };
