...
import { cleanEnv, createFileTree, removeCreatedTrees, ROOT, text, WORK_DIR } from "./helpers/server.ts";
...
describe("stdio", () => {
  ...
  before(async () => {
    ...
    env = { ...cleanEnv(), OLLAMA_URL: ollama.url, FILE_ROOTS: root } as Record<string, string>;
  });

  after(() => ollama.close());

  // Client の versionNegotiation が受け取る値。文字列のままだと union に当たらない
  ...
  test("stdin が閉じると、処理中の Ollama の呼び出しを止めて終わる", async () => {
    ...
    const child = spawn(process.execPath, [path.join(ROOT, "stdio.ts")], {
      cwd: WORK_DIR,
