...
- `list_files`は`pattern`にグロブを取ります。`*`は直下、`**/*.php`は下の階層のPHPのファイルすべてです。秘密のファイルと、`node_modules`、`vendor`、`.git`は出しません
...
| `CF_ACCESS_ALLOWED_EMAILS`               | なし                                                           | 設定すると、AccessのJWTの`email`がこの一覧にある人だけを通します。カンマで区切って並べます                  |
...
1. `docker compose up -d`でコンテナーを作り直します。ログに`File tools are enabled over HTTP`と出れば有効です
1. claude.aiのコネクタを切り直すと、ツールの一覧に`list_files`が加わります
