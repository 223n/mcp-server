...
/** 読み込む 1 件。start と end は `#L10-200` で範囲を指定したときだけ入る */
...
export function readRoots(): Root[] {
...
export function fileRootsLabel(): string {
  return readRoots()
...
async function verifyComponents(realRoot: string, realLocal: string, input: string): Promise<void> {
...
async function resolvePath(input: string): Promise<ResolvedPath> {
...
function toHostPath(local: string, root: Root, realRoot: string): string {
...
export async function listFiles({
  ...
  const queue: Crawl[] = [{ dir: local, relative: "", depth: 0 }];
  ...
  walk: while (queue.length > 0) {
    ...
    // while の条件で空でないことは確かめているが、型の上では undefined になりうる
    if (!item) {
      break;
    }

    const { dir, relative, depth } = item;
...
export function numberLines(lines: string[], startLine = 1): string {
...
export function splitLineRange(input: string): { path: string; start?: number; end?: number } {
...
function expandable(name: string): boolean {
  if (name.startsWith(".")) {
    return false;
  }
...
function splitGlob(input: string): { base: string; pattern: string } {
...
async function expandGlob(input: string, signal?: AbortSignal): Promise<Target[]> {
...
async function resolveItem(input: string, signal?: AbortSignal): Promise<Target[]> {
  if (/[*?{]/.test(input)) {
...
async function readPart(
  { local, display, start, end }: Target,
  { lineNumbers }: { lineNumbers: boolean },
): Promise<Part> {
...
function inlinePart({ name, content }: InlineFile, { lineNumbers }: { lineNumbers: boolean }): Part {
...
export async function readOneFile(
  ...
  // resolveItem は 1 件以上を返すか投げるため、ここには来ない。型の上での念のための確認
  if (!target) {
    throw new Error(`File not found: ${input}`);
  }
 ...
 */
export async function readForResource(
  input: string,
