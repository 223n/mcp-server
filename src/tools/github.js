...
// git.js と同じ形だけを通す。パスに使う文字が混ざらないようにする
...
function trim(text, limit = MAX_BODY_CHARS) {
 ...
 */
async function api(path, { method = "GET", body, accept, signal } = {}) {
  ...
  return accept ? text : JSON.parse(text || "null");
}

const line = (parts) => parts.filter(Boolean).join("  ");

export async function githubRead(args, ctx) {
  ...
  switch (args.op) {
    case "pr_list": {
      ...
      return (
        items
          .map((pr) => line([`#${pr.number}`, pr.state, pr.user?.login, `${pr.head?.ref} -> ${pr.base?.ref}`, pr.title]))
          .join("\n") || "No pull requests matched."
      );
    }

    case "pr_view": {
      const pr = await api(`/repos/${owner}/${repo}/pulls/${number(args.number)}`, { signal });
    ...
    case "pr_diff":
      return trim(
        await api(`/repos/${owner}/${repo}/pulls/${number(args.number)}`, {
    ...
    case "pr_checks": {
      ...
      const runs = await api(`/repos/${owner}/${repo}/commits/${pr.head.sha}/check-runs?per_page=100`, {
        signal,
      });
    ...
    case "issue_view": {
      const issue = await api(`/repos/${owner}/${repo}/issues/${number(args.number)}`, { signal });

      return [`#${issue.number} ${issue.title}`, line([issue.state, `by ${issue.user?.login}`]), "", trim(issue.body) || "(no body)"].join(
        "\n",
      );
    }
...
export async function githubWrite(args, ctx) {
  const { owner, repo } = splitRepo(args.repo);
